...
(defmethod sicl-reader:interpret-token
    (token token-escapes (input-stream stream))
  (sicl-reader::convert-according-to-readtable-case
   token token-escapes)
    ...
    (tagbody
     ...
     symbol
	   ...
	   (cond ((null position-package-marker-1)
                  (let* ((result (intern token *package*))
                         (package (symbol-package result)))
                    (unless (or (null package)
                      ...
                      (format *output-stream*
                              ...
                              *input-filename*
                              ...
                              (- (column-number input-stream)
                                 (length token))
                              token
                              (package-name package)))
                    (return-from sicl-reader:interpret-token result)))
			   ...
			   (cond ((null symbol)
				 ...
				 ((eq status :internal)
				  (error 'symbol-is-not-external
					 ...
					 :desired-symbol token))
				 (t
				  (return-from sicl-reader:interpret-token symbol)))))))
		 (t
		  (if (= position-package-marker-1 (1- length))
		      (error 'symbol-name-must-not-end-with-package-marker
			     :stream input-stream
			     :desired-symbol token)
                      (let* ((sname (subseq token (1+ position-package-marker-2)))
                             (pname (subseq token 0 position-package-marker-1))
                             (result (intern sname pname)))
                        (return-from sicl-reader:interpret-token result)))))
           (let ((char (aref token index)))
	     (cond ((eq (aref token-escapes index) t)
		    (go symbol))
